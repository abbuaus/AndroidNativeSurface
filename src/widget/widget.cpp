...
void widget::CheckBox(std::string text, bool* b)
{
    ...
    const ImGuiID id = window->GetID(text.c_str());//用text生成一个id
    const ImVec2 text_size = ImGui::CalcTextSize(text.c_str(), NULL, true);//text的大小
    const ImRect bb(window->DC.CursorPos, ImVec2(window->DC.CursorPos.x + 80, window->DC.CursorPos.y + 80));//控件的位置

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(style.Colors[ImGuiCol_FrameBg]));//添加一个背景
    if (*b)
    {
        window->DrawList->AddRectFilled(ImVec2( bb.Min.x + 10,bb.Min.y + 10 ),ImVec2( bb.Max.x -10,bb.Max.y -10 ), ImColor(style.Colors[ImGuiCol_CheckMark]));
    }//如果*b为真，添加一个内嵌矩形
    window->DrawList->AddText(ImVec2(bb.Max.x, bb.Max.y - 40 - (text_size.y / 2)), ImColor(style.Colors[ImGuiCol_Text]), text.c_str());//添加文字
    ImGui::ItemAdd(bb, id);//添加item
